        ...
        */
        class PubUtil {
            ...
            */
             ...
             * @param defaultValue
             */
            static setNumber(arrNumber, index, defaultValue) {
             ...
             * @param defaultValue
             */
            static setString(arrString, index, defaultValue) {
    ...
    (function (util) {
        ...
        */
        class MathUtil {
             ...
             * @param toY
             */
            static getTwoPointAngle(fromX, fromY, toX, toY) {
                let radian = Math.atan2(fromY - toY, fromX - toX);
                let angle = MathUtil.radianToAngle(radian);
                //从左到右 -135 -45  135 45
                return angle;
            }
            /**
             ...
             * @param rotate 旋转角度
             */
            static getDirByRotate(rotate) {
                let ret = ActionDirect.Right;
        ...
        class TouchManager {
            ...
            onMouseMove(t) {
                if (this._isTouch) {
                    ...
                    let distance = this._startPoint.distance(this._curPoint.x, this._curPoint.y);
                    if ((this.maxMoveCount == 0 || this._moveCount < this.maxMoveCount) && distance >= this.mouseSpeed) {
                        let radian = Math.atan2(this._curPoint.y - this._startPoint.y, this._curPoint.x - this._startPoint.x);
                        let angle = MathUtil.radianToAngle(radian);
            ...
            class SkillManager {
                ...
                initData(data) {
                    ...
                    for (let skill of skills) {
                        if (skill.atkingCount == 0)
                            skill.atkingCount = 1;
                        //不是散射的情况下，散射角度强制设置为0
                        if (skill.atkingCount == 1) {
                            skill.atkingRotation = 0;
                        }
                        ;
    ...
    (function (scene) {
        ...
        */
        class SceneManager {
            ...
            checkLoopMap() {
                if (this.isLoopMap) {
                    let tempIndex = Math.ceil(this.scene.x / this.loopMapWidth);
                    if (tempIndex < this.loopMapIndex) {
                        this.loopMapIndex = tempIndex;
                        //地图像左
                        let firstMap = this.arrMap.shift();
        ...
        */
        class Scene3DManager {
            constructor() {
                ...
                if (window["Laya3D"]) {
                    this.scene3d = scene.LayerManager.ins.addChild(new Laya.Scene3D(), scene.LayerName.scene_king);
                }
            }
            static get ins() {
                if (!this._ins)
            ...
            initCamera() {
                ...
                //方向光
                let directionlight = this.scene3d.addChild(new Laya.DirectionLight());
    ...
    (function (scene) {
        ...
        */
        class RockerManager {
             ...
             */
            init(rocker, rockerBg, rockerRadis = 0, rockerArea = [0], isFollow = false) {
                ...
                if (!rockerArea || rockerArea.length == 1) {
                    let areaRadis = rockerArea && rockerArea[0] == 0 ? this.rockerBg.width >> 1 : rockerArea[0];
                    graphics.drawCircle(0, 0, areaRadis, "#ffffff");
                }
                else {
                    //2020-04-22 默认屏幕宽度
                    let areaX = rockerArea[0] == 0 ? game.Define.DeviceW : rockerArea[0];
                    let areaY = rockerArea[1] == 0 ? game.Define.DeviceH : rockerArea[1];
                    //全屏滑动区域坐标 和 指定滑动区域坐标
            ...
            onMouseDown(evt) {
                ...
                //2020-04-22 不能超出初始半径
                let localMousePoint = new Laya.Point(Laya.stage.mouseX, Laya.stage.mouseY);
            ...
            onMouseMove(t) {
                if (this._isTouch) {
                    ...
                    let distance = this.lastPoint.distance(this.curPoint.x, this.curPoint.y);
                    if ((this.maxMoveCount == 0 || this.moveCount < this.maxMoveCount) && distance >= this.mouseSpeed) {
                        let radian = Math.atan2(this.curPoint.y - this.startPoint.y, this.curPoint.x - this.startPoint.x);
                        let angle = MathUtil.radianToAngle(radian);
                        angle = MathUtil.changeAngle360(angle);
                        //
    ...
    (function (scene) {
        ...
        */
        class LayerManager {
            ...
            init() {
                game.Global.root_bottom = Laya.stage.addChild(new Laya.Sprite());
                game.Global.root = Laya.stage.addChild(new Laya.Sprite());
                game.Global.root_top = Laya.stage.addChild(new Laya.Sprite());
                //初始化层级
            ...
            */
            class NpcManager {
                ...
                createNpc(cls) {
                    ...
                    let arrNpc = this._dicPool.get(clsName);
                    if (!arrNpc) {
                        arrNpc = [];
                 ...
                 */
                removeNpc(npcID) {
                    let npc = this._dicNpc.get(npcID);
                    if (npc) {
                        this._dicNpc.remove(npc.objId);
                        let arrNpc = this._dicPool.get(npc.clsName);
                        if (arrNpc) {
                            arrNpc.push(npc);
        ...
        (function (king_1) {
            ...
            */
            class KingManager {
                ...
                /**获得动作配置唯一 */
                getCfg(skin, actionType, actionDirect) {
                ...
                /**获得动作配置集合 */
                getActionBySkin(skin) {
                    let arrAction = [];
                    let arrCfg = this.getCfgBySkin(skin);
                ...
                createKing(cls) {
                    ...
                    let king = null;
                    let clsName = cls.name;
                    let arrKing = this._dicPool.get(clsName);
                    if (!arrKing) {
                        arrKing = [];
                 ...
                 */
                removeKing(kingID) {
                    let king = this._dicKing.get(kingID);
                    if (king) {
                        this._dicKing.remove(king.objId);
                        let arrKing = this._dicPool.get(king.clsName);
                        if (arrKing) {
                            arrKing.push(king);
        ...
        (function (king) {
            ...
            */
            class BoneManager {
                 ...
                 */
                createBone(base64Type) {
                    let temp = null;
                    let boneName = base64Type.name;
                    let cacheType = base64Type.paramNum;
                    //模板
    ...
    (function (effect) {
        ...
        */
        class FrameManager {
             ...
             * @param callBack   回调函数
             */
            getFrameBySkin(skin, callBack = null) {
                let cfg = this.getCfgBySkin(skin);
                if (cfg) {
                    return new effect.BaseFrame(cfg, callBack);
                }
                return null;
             ...
             * @param frame
             */
            addFrame(frame, layer = LayerName.main) {
             ...
             * @param skinName
             */
            regAnimation(skinName) {
                let cfg = this.getCfgBySkin(skinName);
                if (!cfg) {
                    console.log("FrameManager.ts 注册动画失败,请检查Frame表：" + skinName);
                    return;
                }
                //创建动画模板
                let arr = BaseAnimation.aniUrls(cfg.skin, cfg.count);
    ...
    (function (effect) {
        ...
        */
        class EffectManager {
             ...
             */
            getEffect(en, effectData, isScene = true) {
                let be = null;
                switch (en) {
             ...
             * @param layer
             */
            addEffect(be, layer = LayerName.main) {
    ...
    (function (common) {
        ...
        */
        class TweenManager {
             ...
             */
            regTween(tt, isReplace = true) {
                if (!tt) {
                    return;
                }
                if (this.dicTween.hasKey(tt.id)) {
                    let ttNow = this.dicTween.get(tt.id);
                    ttNow = tt;
                }
                else {
             ...
             */
            resetTargetPos(name, x, y, isPlay = true) {
                let tt = this.dicTween.get(name);
                if (tt) {
                    tt.stop();
                    tt.target.x = x;
    ...
    (function (common) {
        ...
        */
        class TipManager {
             ...
             */
            showImg(url, x = 0, y = 0, offH = -50, style = 0) {
                let imgEfct;
             ...
             * @param style 显示风格 0.竖直缓动 1.垂直先大后小 默认0
             */
            showWord(msg, x = 0, y = 0, offH = -50, fontSize = 40, fontColor = "#000000", stroke = 0, strokeColor = "", style = 0) {
             ...
             */
            showNumber(num, x = 0, y = 0, offH = -50, style = 0, path = "game/img_number", numW = 30, sp = null) {
             ...
             * @param numW 数字图片容器，默认 null
             */
            numberToImg(num, path = "game/img_number", numW = 30, sp = null) {
...
(function (game) {
    ...
    (function (display) {
         ...
         * 所有显示基类
         */
        class BaseDisplay extends Laya.Sprite {
            constructor() {
                super();
...
(function (game) {
    ...
    (function (display) {
         ...
         * 动画基类 生物，序列帧
         */
        class BaseAnimation extends display.BaseDisplay {
            /**
             *
             * @param frameName
             * @param frameCount
             ...
             * length   动画最后一帧的索引值，
             */
            static aniUrls(skinName, length) {
                var urls = [];
...
(function (game) {
    ...
    (function (display) {
        ...
        * UI基类 窗体
        */
        class BaseUI extends display.BaseDisplay {
            constructor() {
                super();
...
(function (game) {
    ...
    (function (effect) {
        ...
        * 特效基类
        2019-07-17 andy
        */
        class BaseEffect extends BaseUI {
            constructor(isScene) {
                super();
                /**是否正在播放特效 */
...
(function (game) {
    ...
    (function (effect) {
        ...
        * 空投掉落特效
        2020-01-03 andy
        */
        class AirDropEft extends effect.BaseEffect {
            constructor(isScene) {
                super(isScene);
            }
            /**执行一次 */
        ...
        * 空投数据
        2020-01-03 andy
        */
        class AirDropData extends effect.BaseEffectData {
            /**
             *
             * @param planSkin      飞机皮肤
             ...
             */
            constructor(planSkin, parachuteSkin, shadowSkin, dropBoxSkin) {
                super();
                /**飞机飞行时间 默认1000毫秒*/
                this.planeFlyTime = 1000;
                /**飞机旋转角度 默认0*/
                this.planeRotate = 0;
                /**降落伞空投延时时间 默认500毫秒*/
                this.parachuteDelayTime = 500;
                /**降落伞飞行时间 默认1000毫秒*/
                this.parachuteFlyTime = 1000;
                /**降落伞起始缩放 默认1*/
                this.parachuteStartScale = 1;
                /**降落伞目标缩放 默认1*/
                this.parachuteEndScale = 1;
                /**降落伞影子起始缩放 默认1*/
                this.shadowStartScale = 1;
                /**降落伞影子目标缩放 默认1*/
                this.shadowEndScale = 1;
                this.planSkin = planSkin;
                this.parachuteSkin = parachuteSkin;
                this.shadowSkin = shadowSkin;
                this.dropBoxSkin = dropBoxSkin;
                this.planeStartPoint = new Laya.Point(0, game.Define.DeviceH);
                this.planeEndPoint = new Laya.Point(game.Define.DeviceW, 0);
                this.parachuteStartPoint = new Laya.Point(game.Define.DeviceW >> 1, 0);
                this.parachuteEndPoint = new Laya.Point(game.Define.DeviceW >> 1, game.Define.DeviceH >> 1);
                this.shadowStartPoint = new Laya.Point(game.Define.DeviceW >> 1, game.Define.DeviceH);
            }
        }
...
(function (game) {
    ...
    (function (effect) {
         ...
         * 序列帧动画
         */
        class BaseFrame extends BaseAnimation {
            /**
             *
             * @param cfg  Cfg_Frame
             * @param callBack
            ...
            loadFinish() {
                this.isLoad = true;
                //创建动画模板
                let arr = BaseAnimation.aniUrls(this.skin, this.count);
            ...
            setSkin(skin) {
                if (this.anim) {
                    ...
                    let cfg = effect.FrameManager.ins.getCfgBySkin(skin);
                    this.setData(cfg);
                    this.loadFinish();
                    if (cfg == null) {
                        return;
             ...
             */
            playFrame(isLoop = false, parent = null, isRemove = true) {
                this.isLoop = isLoop;
...
(function (game) {
    ...
    (function (effect) {
        ...
        * Boss来袭数据
        2020-01-10 andy
        */
        class BossWarningData extends effect.BaseEffectData {
            constructor(bossSkin, line1Skin, line2Skin, word) {
                super();
                /**交叉线皮肤 垂直间距 默认20*/
                this.linePadding = 20;
                /**Boss皮肤 Y坐标*/
...
(function (game) {
    ...
    (function (effect) {
        ...
        * 喷泉数据
        2020-01-10 andy
        */
        class ChangeImageData extends effect.BaseEffectData {
            constructor() {
                super();
                /**切换时间 单位毫秒,默认1000*/
...
(function (game) {
    ...
    (function (effect) {
        ...
        */
        class FountainEft extends effect.BaseEffect {
            ...
            launch() {
                ...
                img.alpha = 0.6;
                //设置随机值
                let randX = MathUtil.randomRange(this.data.minX, this.data.maxX);
                randX = MathUtil.randomRange(0, 1) == 0 ? randX : -randX;
                let randY = MathUtil.randomRange(this.data.minY, this.data.maxY);
        ...
        * 喷泉数据
        2020-01-10 andy
        */
        class FountainData extends effect.BaseEffectData {
            constructor(imgSkin) {
                super();
                /**发射的时间间隔 默认10毫秒*/
                this.oneSendTime = 10;
                /**飞行的时间间隔 默认200毫秒*/
                this.oneShowTime = 200;
                /**喷出最小X*/
                this.minX = 0;
                /**喷出最大X*/
                this.maxX = 0;
                ...
                this.maxX = game.Define.DeviceW;
                this.maxY = game.Define.DeviceH;
            }
        }
...
(function (game) {
    ...
    (function (effect) {
        ...
        class GuideClickEft extends effect.BaseEffect {
            ...
            big(sp, hand) {
                ...
                Laya.timer.once(200, this, () => {
                    sp.scaleX = 1;
                    sp.scaleY = 1;
                    sp.alpha = this.data.waterAlphaStart;
        ...
        * Boss来袭数据
        2020-01-10 andy
        */
        class GuideClickData extends effect.BaseEffectData {
            constructor(handSkin) {
                super();
                /**手型皮肤*/
                this.handSkin = "";
                /**手型是否剧中 默认0 不居中*/
                ...
                this.waterRadis = 50;
                /**水纹半径放大比例*/
                this.waterRadisRate = 2;
                /**水纹透明度开始值，默认1*/
                this.waterAlphaStart = 1;
                /**水纹透明度结束值，默认0*/
                this.waterAlphaEnd = 0;
                /**水纹颜色*/
...
(function (game) {
    ...
    (function (effect) {
        ...
        * 满屏金币掉落特效
        2019-12-19 andy
        */
        class OutGoldEft extends effect.BaseEffect {
            constructor(isScene) {
                super(isScene);
                //圆形水纹
                this.arrGold = null;
            ...
            setData(effectData) {
                ...
                for (let i = 0; i < this.data.count; i++) {
                    let img = new Laya.Image(this.data.imgSkin);
            ...
            play(parent = null) {
                ...
                for (let i = 0; i < this.data.count; i++) {
                    ...
                    let toX = img.x + (Math.random() < 0.5 ? 1 : -1) * this.data.dropOffx;
        ...
        * 喷金币数据
        2020-01-10 andy
        */
        class OutGoldData extends effect.BaseEffectData {
            constructor(imgSkin, count) {
                super();
                /**金币皮肤*/
                this.imgSkin = "";
                /**产生数量*/
                ...
                this.count = count;
                this.maxX = game.Define.DeviceW;
                this.maxY = game.Define.DeviceH >> 1;
                this.dropY = game.Define.DeviceH;
            }
        }
...
(function (game) {
    ...
    (function (effect) {
        ...
        * 喷出星星特效
        2020-08-11 andy
        */
        class OutStarEft extends effect.BaseEffect {
            constructor(isScene) {
                super(isScene);
                //星星数组
                this.arrStar = null;
            ...
            play(parent = null) {
                ...
                let halfX = game.Define.DeviceW >> 1;
                let radis = 0;
                let showCount = this.data.count;
                for (let i = 0; i < showCount; i++) {
                    let img = this.arrStar[i];
                    img.visible = true;
                    img.scaleX = img.scaleY = this.data.minScale;
                    let radian = MathUtil.angleToRadian((360 / showCount) * i);
                    img.x = Math.cos(radian) * this.data.minRadis;
                    img.y = Math.sin(radian) * this.data.minRadis;
                    let toX = 0;
                    let toY = 0;
                    if (this.data.isRandRadis) {
                        let randRadis = MathUtil.randomRange(this.data.minRadis, this.data.maxRadis);
        ...
        * 喷出星星数据
        2020-08-11 andy
        */
        class OutStarData extends effect.BaseEffectData {
            constructor(imgSkin, count) {
                super();
                /**金币皮肤*/
                this.imgSkin = "";
...
(function (game) {
    ...
    (function (effect) {
        ...
        * 等待特效
        2020-05-21 andy
        */
        class WaitingEft extends effect.BaseEffect {
            constructor(isScene) {
                super(isScene);
                //旋转图片
                this.arrImg = null;
            ...
            setData(effectData) {
                ...
                for (let i = 0; i < this.count; i++) {
                    ...
                    let img = new Laya.Image(this.data.skin + (i % this.data.skinCount + 1) + ".png");
                    this.arrImg.push(img);
                    img.anchorX = img.anchorY = 0.5;
                        ...
                        for (let i = 0; i < this.count; i++) {
                            ...
                            if (this.isZoom) {
                                Laya.Tween.to(img, { x: 0, y: 0, scaleX: 0.5, scaleY: 0.5 }, this.data.zoomSpeed, null, Laya.Handler.create(this, (img) => {
                                    let index = this.arrImg.indexOf(img);
            ...
            addRotation(index, rotation) {
                let radis = this.data.arrNumber[index * 2];
                let angle = this.data.arrNumber[index * 2 + 1];
                angle += rotation;
                this.data.arrNumber[index * 2 + 1] = angle;
                return this.getPosByAngle(radis, angle);
            }
            getPosByAngle(radis, angle) {
                //计算弧度
                let radian = MathUtil.angleToRadian(angle);
                let x = Math.cos(radian) * radis;
                let y = Math.sin(radian) * radis;
                return [x, y];
            }
        }
        ...
        * 等待加载数据
        2020-05-21 andy
        */
        class WaitingData extends effect.BaseEffectData {
            constructor(skin, skinCount = 1, rotationSpeed = 1, zoomSpeed = 200, zoomTime = 1000) {
                super();
                /** 皮肤 */
                this.skin = "";
                /** 皮肤数量 */
...
(function (game) {
    ...
    (function (platform) {
        ...
        * bytedance 国内平台 https://bytedance.feishu.cn/docs/doccnTne26fXXc54DGgWeBaq3Ae#
        2020-11-27 andy
        */
        class ByteDancePlatform extends platform.LocalPlatform {
            constructor() {
                super();
...
(function (game) {
    ...
    (function (platform) {
        ...
        * 腾讯QQ 试玩规范 https://docs.qq.com/doc/DVFZSYUVpREJtZUxJ?pub=1
        2020-11-24 andy
        */
        class QQPlatform extends platform.LocalPlatform {
            constructor() {
                super();
                game.Global.platformId = platform.PlatformID.Qq;
...
(function (game) {
    ...
    (function (platform) {
        ...
        * Tapjoy
        2020-02-21 andy
        */
        class TapjoyPlatform extends platform.LocalPlatform {
            constructor() {
                super();
...
(function (game) {
    ...
    (function (platform) {
        ...
        * TikTok 国内平台 https://ads.tiktok.com
        2020-09-23 andy
        */
        class TiktokPlatform extends platform.LocalPlatform {
            constructor() {
                super();
                game.Global.platformId = platform.PlatformID.Tt;
                game.Define.langId = platform.LangType.En;
                game.Define.isLocal = true;
                game.Define.serverHttp = "http://192.168.2.104:3000/";
            }
        ...
        platform.TiktokPlatform = TiktokPlatform;
    })(platform = game.platform || (game.platform = {}));
})(game || (game = {}));
        ...
        */
        class WxPlatform extends platform.LocalPlatform {
            constructor() {
                ...
                this.wx = Laya.Browser.window.wx;
                this.wxFileMgr = this.wx.getFileSystemManager();
            }
            init(data) {
                super.init(data);
                //初始化微信相关
            ...
            saveImageToPhotosAlbum(filePath) {
                var file = null; //Laya.MiniAdpter.getFileInfo(Laya.URL.basePath+filePath);
                var md5 = file.md5;
...
(function (game) {
    ...
    (function (scene) {
        ...
        (function (king) {
             ...
             * 骨骼基类
             */
            class BaseBone extends BaseDisplay {
                /**
                 * 构造
                 * @param type       骨骼名字
                 * @param cacheType  模板类型
                 */
                constructor(type, cacheType = 0) {
                    super();
                    /**唯一标识ID */
                    this._objId = 0;
                    /**缓存类型 */
                 ...
                 */
                setSkin(boneName, cacheType = 0) {
                    let temp = king.BoneManager.ins.getTemp(boneName);
                    if (!temp) {
                        return;
                    }
                    let sk = temp.buildArmature(cacheType);
                    if (sk) {
                        this._sk = sk;
                 ...
                 */
                play(isLoop = false, name = "", parent = null) {
                    this.isLoop = isLoop;
                    this._randName = name;
                    if (this.isDisplay) {
                    }
...
(function (game) {
    ...
    (function (scene) {
        ...
        (function (king) {
             ...
             * 生物基类
             */
            class BaseKing extends BaseAnimation {
                constructor() {
                    super();
                    /**生物全局唯一标识ID */
                    this.objId = 0;
                 ...
                 * @param action
                 */
                regNextAction(atcionType, actionNaxtType) {
                ...
                setActionType(at, isLoop = true) {
                    ...
                    let actionName = this.skinData.kingType + "_" + at + "_" + this.curActionDirect;
                    ...
                    if (this.isLoad) {
                        ...
                        let action = this.dicAction.get(at + "_" + this.curActionDirect);
                        if (action) {
...
(function (game) {
    ...
    (function (scene) {
        ...
        (function (king_1) {
            ...
            */
            class King extends king_1.BaseKing {
                constructor() {
                    super();
                    /**攻击距离 */
                    this.atkDistance = 1;
                ...
                setSkin(skinData, isAdd = true) {
                    ...
                    let arrSkillConfig = SkillManager.ins.getKingCfg(this.skinData.kingType);
                    if (arrSkillConfig) {
                        //物理
                        let cfgAtk = null;
                        //技能
                ...
                move() {
                    if (game.Define.gameType == game.GameType.over_look) { //俯视角度
                        let angle = MathUtil.getTwoPointAngle(this.x, this.y, this.endPoint.x, this.endPoint.y);
                        this.spBody.rotation = MathUtil.getFromAngle(angle);
                        let radian = MathUtil.angleToRadian(this.spBody.rotation);
                        ...
                        let angle = MathUtil.getTwoPointAngle(this.x, this.y, this.endPoint.x, this.endPoint.y);
                        let realAngle = MathUtil.getFromAngle(angle);
                        let dir = MathUtil.getDirByRotate(realAngle);
                        if (this.setActionDirect(dir)) {
                            this.setActionType(this.curActionType);
                        }
                        let radian = MathUtil.angleToRadian(realAngle);
                        this.x += Math.cos(radian) * this.moveSpeed;
                        this.y += Math.sin(radian) * this.moveSpeed;
                        if (!this.isMoveByDistance()) {
                            this.attack();
                ...
                /**是否需要移动,目标超出攻击距离则需要*/
                isMoveByDistance() {
                    let distance = MathUtil.getDistance(this.startPoint, this.endPoint);
                    if (this.arrFixedPoint && this.curFixedIndex < this.arrFixedPoint.length) {
                 ...
                 */
                checkMoveDir(fromX, fromY, toX, toY, isEightDir = true) {
                    let radian = Math.atan2(fromY - toY, fromX - toX);
                    let angle = MathUtil.radianToAngle(radian);
                    //从左到右 -135 -45  135 45
                ...
                attack(king = null) {
                    ...
                    let tempActionState = this.curActionState;
                ...
                playSkill(target, skillId = -1) {
                    ...
                    let skillConfig = SkillManager.ins.getCfg(skillId);
                    if (!skillConfig) {
                        console.warn("King.ts playSkill ：技能ID " + skillId + " 找不到配置！");
                        return false;
                    }
                    //物理攻击，公用一个CD,key是skill_0
                    let key = "skill_" + (skillConfig.skillType > 0 ? skillId : 0);
                    ...
                    if (Laya.timer.currTimer - this.lastAtkTime >= skillConfig.atkCD) {
                        ...
                        //是否出发暴击
                        let rate = MathUtil.randomRange(1, 10000);
                        ...
                        //播放技能特效
                        let skillData = new SkillData(skillConfig, this, target);
                        let skill = SkillManager.ins.getSkill(skillData);
                        skill.play();
                        return true;
                    }
                    else {
                 ...
                 */
                attacked(king, skillId = 0, extraAtk = 0) {
                 ...
                 *
                 */
                getAtkingEftRotation(atkingX = 0, atkingY = 0) {
                    //方向角度
                    let dirRotation = 0;
                    ...
                    if (game.Define.gameType != game.GameType.over_look && this.atkTarget) {
                        ...
                        let angle = MathUtil.getTwoPointAngle(this.x + atkingX, this.y + atkingY, this.atkTarget.x + this.atkTarget.atkedX, this.atkTarget.y + this.atkTarget.atkedY);
...
(function (game) {
    ...
    (function (scene) {
        ...
        (function (king) {
            ...
            */
            class Npc extends king.BaseKing {
                constructor() {
                    super();
                    /**攻击距离 */
                    this.atkDistance = 1;
                ...
                move() {
                    if (game.Define.gameType == game.GameType.over_look) {
                        let angle = MathUtil.getTwoPointAngle(this.x, this.y, this.atkTarget.x, this.atkTarget.y);
                        this.spBody.rotation = MathUtil.getFromAngle(angle);
                        let radian = MathUtil.angleToRadian(this.spBody.rotation);
                        this.x += Math.cos(radian) * this.moveSpeed;
                        this.y += Math.sin(radian) * this.moveSpeed;
...
(function (game) {
    ...
    (function (scene) {
        ...
        (function (skill) {
             ...
             * 技能基类
             */
            class BaseSkill extends BaseDisplay {
                constructor() {
                    super();
                    this.startPoint = new Laya.Point();
                    this.endPoint = new Laya.Point();
             ...
             */
            class SkillData {
                constructor(Cfg_Skill, atkTarget, atkedTarget) {
...
(function (game) {
    ...
    (function (scene) {
        ...
        (function (skill) {
             ...
             * 普通攻击技能
             */
            class AttackSkill extends skill.BaseSkill {
                constructor() {
                    super();
                }
                /**
...
(function (game) {
    ...
    (function (scene) {
        ...
        (function (skill) {
             ...
             * 子弹技能
             */
            class BulletSkill extends skill.BaseSkill {
                constructor() {
                    super();
                    /**自动导航*/
                ...
                play() {
                    if (this.atkTarget) {
                        ...
                        if (this.atkingEft) {
                            let bulletCount = this.cfgSkill.atkingCount;
                            let bulletRotation = this.cfgSkill.atkingRotation;
                            let minRotation = this.atkTarget.getAtkingEftRotation(this.cfgSkill.atkingX, this.cfgSkill.atkingY) - (bulletRotation >> 1);
                            let addRotation = 0;
                            if (bulletCount > 1) {
                ...
                createBullet(bullet, rotation) {
                    //子弹旋转角度
                    bullet.rotation = rotation;
                    //子弹场景发出坐标
                    let radian = MathUtil.angleToRadian(bullet.rotation);
                    ...
                    let toX = bullet.x + this.cfgSkill.atkDistance * Math.cos(radian);
                    let toY = bullet.y + this.cfgSkill.atkDistance * Math.sin(radian);
                    //匀速射击，根据距离计算射击时间
                ...
                /**发射结束 */
                atkingFinish(bullet, enemy = null) {
                    //发射结束
                    if (this.atkEft)
...
(function (game) {
    ...
    (function (scene) {
        ...
        (function (skill) {
             ...
             * 群杀技能
             */
            class GroupKillSkill extends skill.BaseSkill {
                constructor() {
                    super();
                }
                /**
                ...
                play() {
                    if (this.atkTarget) {
                        ...
                        if (this.atkingEft) {
                            ...
                            //子弹场景发出坐标
                            let radian = MathUtil.angleToRadian(this.atkingEft.rotation);
                            ...
                            let toX = this.atkedTarget ? this.atkedTarget.x : this.atkTarget.x + game.Define.DeviceW * Math.cos(radian);
                            let toY = this.atkedTarget ? this.atkedTarget.y : this.atkTarget.y + game.Define.DeviceW * Math.sin(radian);
                            //匀速射击，根据距离计算射击时间
        ...
        */
        class BaseWindow extends BaseUI {
            ...
            open(delayTime = 0) {
                ...
                if (delayTime > 0) {
                    let sp = new Sprite();
                    sp.graphics.drawRect(0, 0, game.Define.DeviceW, game.Define.DeviceH, "#ffffff");
...
(function (game) {
    ...
    */
    ...
    /**是否测试模式 */
    Define.isDebug = true;
    /** 设备可视宽度 等同 Laya.Browser.clientWidth*/
    Define.clientWidth = 360;
    /** 设备可视高度 等同 Laya.Browser.clientHeight*/
    Define.clientHeight = 640;
    /** 设备像素密度 等同 Laya.Browser.pixelRatio*/
    Define.pixelRatio = 3;
    /** 设备物理宽度 等同 Laya.Browser.width*/
    Define.width = 1080;
    /** 设备物理高度 等同 Laya.Browser.height*/
    Define.height = 1920;
    /** 游戏设计宽度 等同 Laya.stage.width*/
    Define.DeviceW = 720;
    /** 游戏设计高度 等同 Laya.stage.height*/
    Define.DeviceH = 1280;
    /** 游戏画布宽度 */
    Define.canvasWidth = 1080;
    /** 游戏画布高度 */
    Define.canvasHeight = 1280;
    /**游戏资源根目录 */
    Define.CDN = "res";
    /**服务端HTTP */
